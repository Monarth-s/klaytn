...
VINITN_VERSION=$(go run build/rpm/main.go version)
VINITN_RELEASE_NUM=$(go run build/rpm/main.go release_num)
PACKAGE_SUFFIX="${VINITN_VERSION}-${VINITN_RELEASE_NUM}-${PLATFORM_SUFFIX}.tar.gz"
...
for b in ${DAEMON_BINARIES[*]}; do
    if [ "$TARGET" == "$b" ]; then
        ...
        VINITN_PACKAGE_NAME="${b}${BAOBAB}-${PACKAGE_SUFFIX}"
        DAEMON=1
    fi
done
...
# Compress!
mkdir -p packages
tar czf packages/$VINITN_PACKAGE_NAME $PACK_NAME
