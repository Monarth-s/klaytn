...
type weightedCouncil struct {
	subSize           uint64
	demotedValidators istanbul.Validators // validators staking VINIs less than minimum, and not in committee/proposers
	validators        istanbul.Validators // validators staking VINIs more than and equals to minimum, and in committee/proposers
...
// If no validator has enough VINIs, all become validators.
func filterValidators(isSingleMode bool, govNodeAddr common.Address, weightedValidators []*weightedValidator, stakingAmounts []float64, minStaking uint64) ([]*weightedValidator, []float64, []*weightedValidator, []float64) {
	...
	// when no validator has more than minimum staking amount of VINIs, all members are in validators
	if len(newWeightedValidators) <= 0 {
		// 1. if governance mode is not single,
		// 2. if governance mode is single and governing node does not have minimum staking amount of VINIs as well
