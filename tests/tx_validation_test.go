...
// TestInvalidBalance generates invalid txs which don't have enough VINI, and will be invalidated during txPool insert process.
func TestInvalidBalance(t *testing.T) {
	...
	// test account will be lack of VINI
	...
	// generate a test account with a specific amount of VINI
	{
		var txs types.Transactions
	...
	for _, testTxType := range testTxTypes {
		...
		if !txType.IsFeeDelegatedTransaction() {
			// tx with a specific amount or a gasLimit requiring more VINI than the sender has.
			{
			...
			// tx with a specific amount or a gasLimit requiring the exact VINI the sender has.
			{
				valueMap, _ := genMapForTxTypes(testAcc, reservoir, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
				...
				// the sender can send txs to txpool in multiple times (by the for loop) with limited VINI.
				err = txpool.AddRemote(tx)
				assert.Equal(t, nil, err)
				testAcc.AddNonce()
			}
		...
		if txType.IsFeeDelegatedTransaction() && !txType.IsFeeDelegatedWithRatioTransaction() {
			// tx with a specific amount requiring more VINI than the sender has.
			{
				valueMap, _ := genMapForTxTypes(testAcc, reservoir, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
			...
			// tx with a specific gasLimit (or amount) requiring more VINI than the feePayer has.
			{
			...
			// tx with a specific amount requiring the exact VINI the sender has.
			{
				valueMap, _ := genMapForTxTypes(testAcc, reservoir, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
			...
			// tx with a specific gasLimit (or amount) requiring the exact VINI the feePayer has.
			{
				valueMap, _ := genMapForTxTypes(reservoir, testAcc, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
		...
		if txType.IsFeeDelegatedWithRatioTransaction() {
			// tx with a specific amount and a gasLimit requiring more VINI than the sender has.
			{
				valueMap, _ := genMapForTxTypes(testAcc, reservoir, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
			...
			// tx with a specific amount and a gasLimit requiring more VINI than the feePayer has.
			{
				valueMap, _ := genMapForTxTypes(reservoir, testAcc, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
			...
			// tx with a specific amount and a gasLimit requiring the exact VINI the sender has.
			{
				valueMap, _ := genMapForTxTypes(testAcc, reservoir, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
			...
			// tx with a specific amount and a gasLimit requiring the exact VINI the feePayer has.
			{
				valueMap, _ := genMapForTxTypes(reservoir, testAcc, txType)
				if toBasicType(txType) == types.TxTypeSmartContractExecution {
					valueMap[types.TxValueKeyTo] = contract.Addr
...
// TestInvalidBalanceBlockTx generates invalid txs which don't have enough VINI, and will be invalidated during block insert process.
...
func TestValidationPoolResetAfterFeePayerKeyChange(t *testing.T) {
	...
	// transfer VINI to fee payer
	{
		...
		values := map[types.TxValueKeyType]interface{}{
			...
			types.TxValueKeyAmount:   new(big.Int).Mul(big.NewInt(params.VINI), big.NewInt(100000)),
			types.TxValueKeyGasLimit: gasLimit,
			types.TxValueKeyGasPrice: big.NewInt(25 * params.Ston),
		}
