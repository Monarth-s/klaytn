...
import (
	...
	istanbulCore "github.com/Monarth-s/klaytn/consensus/istanbul/core"
...
func initBlockChain(db database.DBManager, cacheConfig *blockchain.CacheConfig, coinbaseAddrs []*common.Address, validators []common.Address,
	...
	for _, a := range coinbaseAddrs {
		alloc[*a] = blockchain.GenesisAccount{Balance: new(big.Int).Mul(big.NewInt(1e16), big.NewInt(params.VINI))}
	}
