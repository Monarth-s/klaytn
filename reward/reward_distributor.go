...
// this behaves similar to the previous MintVINI
// MintVINI has been superseded because we need to split reward distribution
...
func CalcDeferredRewardSimple(header *types.Header, rules params.Rules, pset *params.GovParamSet) (*RewardSpec, error) {
	...
	// of `deferredTxFee` flag. See https://github.com/Monarth-s/klaytn/issues/1692.
...
func calcShares(stakingInfo *StakingInfo, stakeReward *big.Int, minStake uint64) (map[common.Address]*big.Int, *big.Int) {
	...
	for _, node := range cns.GetAllNodes() {
		if node.StakingAmount > minStake {
			...
			// The VINI unit will cancel out:
			// rewardAmount (peb) = stakeReward (peb) * effectiveStake (VINI) / totalStakes (VINI)
