...
import (
	...
	"github.com/Monarth-s/klaytn/work/mocks"
...
func TestGetRewardsAccumulated(t *testing.T) {
	...
	blockMinted, _ := new(big.Int).SetString("9600000000000000000", 10)  // 9.6 VINI
	blockProposer, _ := new(big.Int).SetString("960000000000000000", 10) // 0.96 VINI = 9.6 VINI * 0.5 * 0.2
	blockStaking, _ := new(big.Int).SetString("3840000000000000000", 10) // 3.84 VINI = 9.6 VINI * 0.5 * 0.8
	blockTxFee, _ := new(big.Int).SetString("25000000000000", 10)        // 25000 Ston = 1000 * 25 Ston
	blockTxBurnt := blockTxFee
	blockKFF, _ := new(big.Int).SetString("1920000000000000000", 10) //  1.92 VINI = 9.6 VINI * 0.2
	blockKCF, _ := new(big.Int).SetString("2880000000000000000", 10) //  2.88 VINI = 9.6 VINI * 0.3
