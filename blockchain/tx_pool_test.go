...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.VINI))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.VINI))
...
func TestHomesteadTransaction(t *testing.T) {
	...
	testAddBalance(pool, from, new(big.Int).Mul(big.NewInt(10), big.NewInt(params.VINI)))
